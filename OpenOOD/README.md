# Theoretically Grounded SCOD - OpenOOD Fork
This repository contains a Python library for evaluating OOD detection methods on a real-world data benchmark. It is a fork of an existing project OpenOOD. The OpenOOD benchmark is not a part of our contributions, however, we provide our fork along with the remaining code to enable reviewers to reproduce our results. We make modifications to the benchmark required to utilize a separate model as a likelihood ratio estimator, e.g., we implement a separate postprocessor for the likelihood estimation.

To generate the scores required for evaluation, we provide scripts:
- scripts/eval_mixture_likelihood_ratio_ood.sh
- scripts/eval_likelihood_ratio_ood.sh
- scripts/eval_posthoc_ood.sh

The first and second scripts evaluate the likelihood ratio estimators, which need to be trained using the `SCOD-Likelihood-Estimation` repository.
The last script evaluated the single-score posthoc methods implemented in OpenOOD, which we use in our experiments.

The remainder of this `README.md` is copied from the original OpenOOD benchmark.
