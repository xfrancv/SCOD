...
class SigmoidClassifier(nn.Module):
    ...
    def forward(self, x):
        pid = self.model(x)
        log_prob_ood = torch.log(1-pid)        
        log_prob_id = torch.log(pid)
        return torch.stack([log_prob_ood, log_prob_id], axis=1)
...
for subfolder in sorted(glob(os.path.join(root, 's*'))):
    ...
    if args.jit_path is not None:
        if 'fold' in args.jit_path:            
            if args.jit_path.split('fold_')[1].split('.pkl')[0] != subfolder.split('/s')[1]:
                continue
    ...
    # If a path to JIT serialized model is specified, we overwrite the model
    # The loaded model need not serve as a classifier, it can only serve as
    # a component for a scoring function
    if args.jit_path is not None:
        net = SigmoidClassifier(torch.jit.load(args.jit_path).cuda())
        postprocessor = get_postprocessor(os.path.join(ROOT_DIR, 'configs'), postprocessor_name, args.id_data)
        postprocessor_name = args.jit_path.split('Classifier/')[1].split('.pkl')[0] + '_' + postprocessor_name        
    else:
