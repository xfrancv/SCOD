...
def build_path_json(root: str):
    ...
    for file_path in tqdm(files_grabbed):
        label = int('cifar100/' in file_path)
    ...
    for i in indexes:
        if outer_dict[i]['label'] == 0:
            if np.random.rand() <= 0.5:# and False:  # cifar, svhn, tin have separate train split. Needs to be switched depending on the OOD dataset
                outer_dict[i]['folder'] = -1
            else:
                outer_dict[i]['folder'] = c_folder        
                c_folder += 1
        else:
            if np.random.rand() <= perc / 2.:# or True: # Needs to be switched depending on the OOD dataset
                outer_dict[i]['folder'] = c_folder        
                c_folder += 1
            else:
