...
def build_path_json(root: str):
    ...
    if nr_out >= nr_in:
        ...
        for folder in range(n_folders):
            ...
            num_to_use = int(min(counts[folder]['ood'], len(valid_indexes)/2))        
            for j in range(num_to_use):    
                outer_dict[valid_indexes[j]]['folder'] = folder
            for j in range(num_to_use, 2*num_to_use):            
                outer_dict[valid_indexes[j]]['folder'] = folder
